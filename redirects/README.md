...
Collection of External Redirects let for configuration by the user.
...
```bash
helm upgrade --install redirects solo-laboratories/external-redirects -n redirects --create-namespace -f values.yaml --version 1.0.0 --atomic
```

### Locally
```bash
helm upgrade --install redirects . -n redirects --create-namespace  --atomic
```
...
```bash
helm uninstall redirects -n redirects
```